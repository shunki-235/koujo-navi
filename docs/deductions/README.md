...
- 入力値のバリデーション、計算結果のブレークダウン表示、PDF/JSONエクスポート。
- 非対応(初期): e-Tax連携、セルフメディケーション税制、住民税側計算、配偶者/扶養控除、高齢者/障害者特例など。
...
### 画面一覧
- `/deductions/flow` ステッパー: 基本情報 → 各控除入力(タブ/ステップ) → 確認 → 結果（控除合計・内訳・注記・PDF/JSONエクスポートを含む）。
